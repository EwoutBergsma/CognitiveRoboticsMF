...
def everyday_shuffling(X_train, y_train):
...
def active_learning_thres(X_train, y_train, X_test, y_test, ini_amount_depth_data_thres = num_trees, num_trees=num_trees, threshold=0.5):
    ...
    rows_remaining_data = [i for i in range(0, X_train.shape[0]) if i not in init_training_data]
    ...
    remaining_X_data = X_train[rows_remaining_data, :]
    remaining_y_data = y_train[rows_remaining_data]
    remaining_all_data = np.concatenate((remaining_X_data, remaining_y_data.reshape((-1, 1))), axis=1)
    # TODO : is there any better way to do this? :(
    selected_X_data = []
    selected_y_data = []
    for instance in remaining_all_data:
...
def active_learning_perc(X_train, y_train, X_test, y_test, ini_n_data_perc = ini_amount_depth_data_perc, num_trees=num_trees, percent=0.5):
    # first, get the initial data set
    shuffled_X_train, shuffled_y_train = everyday_shuffling(X_train, y_train)
    # first round of adaptation training
    mfc = MondrianForestClassifier(n_estimators=num_trees)
    mfc.partial_fit(shuffled_X_train[:ini_n_data_perc+1], shuffled_y_train[:ini_n_data_perc+1])
    all_probabilities = mfc.predict_proba([remaining_all_data_[:, :-1]])
    # get proba-> calculate_conf -> sort -> get some data based on percentage
